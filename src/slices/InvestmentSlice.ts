...
export const createInvestorCosting = createAsyncThunk(
  ...
  async (payload: {name: string,number: number,period: string, interest_rate: number, min_value: number, max_value: number}) => {
      const response = await postRequest(`/plans/create-plan`, payload) as any
